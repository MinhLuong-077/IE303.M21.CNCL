            ...
            public void onClick(View view) {
                ...
                txt_amount =  Long.parseLong(amount1.getText().toString());
                     if((txt_amount>=minimumDepositMoney) && (txt_amount<= maxDepositMoney)){
                         ...
                         {
                             Toast.makeText(Deposit.this,"The balance in the bank is not enough", Toast.LENGTH_SHORT).show();

                         }
                     }
                     else{
                         Toast.makeText(Deposit.this,"Maximum deposit is 10.000.000 and minimum is 10.000", Toast.LENGTH_SHORT).show();
                     }
...
//    }
    ...
    private void checkBankUser() {
        DatabaseReference mDatabase = FirebaseDatabase.getInstance().getReference().child("Bank").child(userFirebase.getUid());
        mDatabase.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (!snapshot.exists()) {
